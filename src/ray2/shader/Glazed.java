...
public class Glazed extends Shader {
	...
	public void shade(Colord outIntensity, Scene scene, Ray ray, IntersectionRecord record, int depth) {
		...
		double R = fresnel(N, V, this.refractiveIndex);
		
		Colord reflectedIntensity = new Colord();
		Colord substrateIntensity = new Colord();
		substrate.shade(substrateIntensity, scene, ray, record, depth+1);
		...
		outIntensity.set(substrateIntensity.mul(1-R).add(reflectedIntensity.mul(R)));
